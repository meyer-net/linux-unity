...
function setup_$soft_name()
{
	...
	# 自定义安装
	# 参数1：安装路径
	function _setup_$soft_name_move()
	{
		# 直装模式
		cd $(dirname ${TMP_$soft_upper_short_name_EXTRA_DIR})
		
		# 轻量级安装的情况下不进行安装包
		## 展开路径还原操作
		mv ${TMP_$soft_upper_short_name_EXTRA_DIR} ${TMP_$soft_upper_short_name_SETUP_DIR}
	}

    # 创建安装目录(纯属为了规范)
    soft_path_restore_confirm_custom "${TMP_$soft_upper_short_name_SETUP_DIR}" "_setup_$soft_name_move"
