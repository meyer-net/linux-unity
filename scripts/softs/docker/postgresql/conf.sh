...
EOF
    ...
    rsync -av ${TMP_DC_PSQ_CONF_SETUP_DIR}/data_replicate/ ${TMP_DC_PSQ_CONF_LNK_DATA_DIR} 
    ...
    local TMP_DC_PSQ_CONF_LNK_ETC_RECV_FILE="postgresql.conf"
    if [ $(echo "${TMP_DC_PSQ_CONF_CTN_SOFT_VER%.*} < 12" | bc) == 1 ]; then
        # 反向link，不然在容器中检测不到文件路径
        #### /mountdisk/etc/docker_apps/library_postgres/imgver111111/data/recovery.conf -> /mountdisk/data/docker_apps/library_postgres/imgver111111/recovery.conf
        path_not_exists_link "${TMP_DC_PSQ_CONF_LNK_ETC_DIR}/recovery.conf" "" "${TMP_DC_PSQ_CONF_LNK_DATA_DIR}/recovery.conf"
        
        TMP_DC_PSQ_CONF_LNK_ETC_RECV_FILE="recovery.conf"
        file_content_not_exists_echo "^standby_mode =.*" "${TMP_DC_PSQ_CONF_LNK_ETC_DIR}/${TMP_DC_PSQ_CONF_LNK_ETC_RECV_FILE}" "standby_mode = on"
    else
        echo > ${TMP_DC_PSQ_CONF_LNK_DATA_DIR}/standby.signal
        file_content_not_exists_echo "^hot_standby =.*" "${TMP_DC_PSQ_CONF_LNK_ETC_DIR}/${TMP_DC_PSQ_CONF_LNK_ETC_RECV_FILE}" "hot_standby = on"
    fi

    file_content_not_exists_echo "^# Defind for" ${TMP_DC_PSQ_CONF_LNK_ETC_DIR}/${TMP_DC_PSQ_CONF_LNK_ETC_RECV_FILE} "# Defind for slave set by meyer.cheng, at ${LOCAL_TIME}"
    file_content_not_exists_echo "^recovery_target_timeline =.*" "${TMP_DC_PSQ_CONF_LNK_ETC_DIR}/${TMP_DC_PSQ_CONF_LNK_ETC_RECV_FILE}" "recovery_target_timeline = 'latest'"
    file_content_not_exists_echo "^primary_conninfo =.*" "${TMP_DC_PSQ_CONF_LNK_ETC_DIR}/${TMP_DC_PSQ_CONF_LNK_ETC_RECV_FILE}" "primary_conninfo = 'host=${TMP_DC_PSQ_CONF_DB_SLAVE_MASTER_HOST} port=${TMP_DC_PSQ_CONF_DB_SLAVE_MASTER_PORT} user=rep_user password=${TMP_DC_PSQ_CONF_DB_SLAVE_PASSWD}'"
...
{    
    ...
    if [ -n "${TMP_DC_PSQ_CONF_CTN_CHOICE}" ]; then
        ...
        {
            TMP_DC_PSQ_CONF_LNK_DATA_DIR=${DOCKER_APP_SETUP_DIR}/${3/\//_}/${4}/data
