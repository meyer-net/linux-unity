...
# rm -rf /opt/docker_apps/library_postgres* && rm -rf /mountdisk/etc/docker_apps/library_postgres* && rm -rf /mountdisk/logs/docker_apps/library_postgres* && rm -rf /mountdisk/data/docker_apps/library_postgres* && rm -rf /opt/docker/data/apps/library_postgres* && rm -rf /opt/docker/etc/library_postgres* && rm -rf /opt/docker/logs/library_postgres* && rm -rf /mountdisk/repo/migrate/clean/library_postgres*
...
function formal_dc_library_postgres() {
    ...
    #### /mountdisk/etc/docker_apps/library_postgres/imgver111111/container
    local TMP_DC_PSQ_SETUP_LNK_ETC_CTN_DIR="${TMP_DC_PSQ_SETUP_LNK_ETC_DIR}/container"
    #### /mountdisk/etc/docker_apps/library_postgres/imgver111111
    function _formal_dc_library_postgres_cp_etc() {
        echo "${TMP_SPLITER2}"
        ...
        #### /mountdisk/data/docker/containers/${CTN_ID} ©&<- /mountdisk/etc/docker_apps/library_postgres/imgver111111/container
        soft_path_restore_confirm_swap "${TMP_DC_PSQ_SETUP_LNK_ETC_CTN_DIR}" "${TMP_DC_PSQ_SETUP_CTN_DIR}"
    }
    soft_path_restore_confirm_create "${TMP_DC_PSQ_SETUP_LNK_ETC_DIR}" "_formal_dc_library_postgres_cp_etc"

    #### /mountdisk/etc/docker_apps/library_postgres/imgver111111/data
    function _formal_dc_library_postgres_lnk_etc() {
        echo "${TMP_SPLITER2}"
        echo_style_text "View the 'etc link'↓:"

        # 废弃，容器中无法检测到路径
        # #### /mountdisk/data/docker_apps/library_postgres/imgver111111/postgresql.conf -> /mountdisk/etc/docker_apps/library_postgres/imgver111111/data/postgresql.conf
        # path_swap_link "${1}/postgresql.conf" "${TMP_DC_PSQ_SETUP_LNK_DATA_DIR}/postgresql.conf"
        # #### /mountdisk/data/docker_apps/library_postgres/imgver111111/pg_hba.conf -> /mountdisk/etc/docker_apps/library_postgres/imgver111111/data/pg_hba.conf
        # path_swap_link "${1}/pg_hba.conf" "${TMP_DC_PSQ_SETUP_LNK_DATA_DIR}/pg_hba.conf"
        # #### /mountdisk/data/docker_apps/library_postgres/imgver111111/pg_ident.conf -> /mountdisk/etc/docker_apps/library_postgres/imgver111111/data/pg_ident.conf
        # path_swap_link "${1}/pg_ident.conf" "${TMP_DC_PSQ_SETUP_LNK_DATA_DIR}/pg_ident.conf"

        # 反向link，不然在容器中检测不到文件路径
        #### /mountdisk/etc/docker_apps/library_postgres/imgver111111/data/postgresql.conf -> /mountdisk/data/docker_apps/library_postgres/imgver111111/postgresql.conf
        path_not_exists_link "${1}/postgresql.conf" "" "${TMP_DC_PSQ_SETUP_LNK_DATA_DIR}/postgresql.conf"
        #### /mountdisk/etc/docker_apps/library_postgres/imgver111111/data/pg_hba.conf -> /mountdisk/data/docker_apps/library_postgres/imgver111111/pg_hba.conf
        path_not_exists_link "${1}/pg_hba.conf" "" "${TMP_DC_PSQ_SETUP_LNK_DATA_DIR}/pg_hba.conf"
        #### /mountdisk/etc/docker_apps/library_postgres/imgver111111/data/pg_ident.conf -> /mountdisk/data/docker_apps/library_postgres/imgver111111/pg_ident.conf
        path_not_exists_link "${1}/pg_ident.conf" "" "${TMP_DC_PSQ_SETUP_LNK_DATA_DIR}/pg_ident.conf"

        ls -lia ${1}
    }
    soft_path_restore_confirm_pcreate "${TMP_DC_PSQ_SETUP_LNK_ETC_DIR}/data" "_formal_dc_library_postgres_lnk_etc"
    ...
    path_not_exists_link "${DOCKER_SETUP_DIR}/logs/${TMP_DC_PSQ_SETUP_IMG_MARK_NAME}/${TMP_DC_PSQ_SETUP_CTN_VER}" "" "${TMP_DC_PSQ_SETUP_LNK_LOGS_DIR}"
    #### /mountdisk/logs/docker_apps/library_postgres/imgver111111/container/${CTN_ID}-json.log -> /mountdisk/etc/docker_apps/library_postgres/imgver111111/container/${CTN_ID}-json.log
    path_not_exists_link "${TMP_DC_PSQ_SETUP_LNK_LOGS_DIR}/container/${TMP_DC_PSQ_SETUP_CTN_ID}-json.log" "" "${TMP_DC_PSQ_SETUP_LNK_ETC_CTN_DIR}/${TMP_DC_PSQ_SETUP_CTN_ID}-json.log"
    ### 数据
    ...
    path_not_exists_link "${DOCKER_SETUP_DIR}/data/apps/${TMP_DC_PSQ_SETUP_IMG_MARK_NAME}/${TMP_DC_PSQ_SETUP_CTN_VER}" "" "${TMP_DC_PSQ_SETUP_LNK_DATA_DIR}"
    ### ETC
    #### /opt/docker_apps/library_postgres/imgver111111/etc -> /mountdisk/etc/docker_apps/library_postgres/imgver111111
    path_not_exists_link "${TMP_DC_PSQ_SETUP_ETC_DIR}" "" "${TMP_DC_PSQ_SETUP_LNK_ETC_DIR}"
    #### /opt/docker/etc/library_postgres/imgver111111 -> /mountdisk/etc/docker_apps/library_postgres/imgver111111
    path_not_exists_link "${DOCKER_SETUP_DIR}/etc/${TMP_DC_PSQ_SETUP_IMG_MARK_NAME}/${TMP_DC_PSQ_SETUP_CTN_VER}" "" "${TMP_DC_PSQ_SETUP_LNK_ETC_DIR}"
    #### /mountdisk/data/docker/containers/${CTN_ID} -> /mountdisk/etc/docker_apps/library_postgres/imgver111111/container
    path_not_exists_link "${TMP_DC_PSQ_SETUP_CTN_DIR}" "" "${TMP_DC_PSQ_SETUP_LNK_ETC_CTN_DIR}"
    ...
    #
    # ${TMP_DC_PSQ_SETUP_WORK_DIR}:/usr/lib/${TMP_DC_PSQ_SETUP_APP_MARK}"
    # ${TMP_DC_PSQ_SETUP_LNK_LOGS_DIR}/app:/var/log/${TMP_DC_PSQ_SETUP_APP_MARK}"
    # ${TMP_DC_PSQ_SETUP_LNK_LOGS_DIR}/app:/usr/lib/${TMP_DC_PSQ_SETUP_APP_MARK}/${TMP_DC_PSQ_SETUP_LOGS_MARK}"
    # ${TMP_DC_PSQ_SETUP_LNK_DATA_DIR}:/usr/lib/${TMP_DC_PSQ_SETUP_APP_MARK}/${TMP_DC_PSQ_SETUP_DATA_MARK}"
    # ${TMP_DC_PSQ_SETUP_LNK_DATA_DIR}:/var/lib/${TMP_DC_PSQ_SETUP_APP_MARK}"
    # ${TMP_DC_PSQ_SETUP_LNK_ETC_DIR}/app:/usr/lib/${TMP_DC_PSQ_SETUP_APP_MARK}/${TMP_DC_PSQ_SETUP_ETC_MARK}
    # ${TMP_DC_PSQ_SETUP_LNK_ETC_DIR}/app:/etc/${TMP_DC_PSQ_SETUP_APP_MARK}
    echo "${TMP_SPLITER2}"
    ...
    docker_change_container_volume_migrate "${TMP_DC_PSQ_SETUP_CTN_ID}" "${TMP_DC_PSQ_SETUP_LNK_LOGS_DIR}/app:/var/log/${TMP_DC_PSQ_SETUP_APP_MARK} ${TMP_DC_PSQ_SETUP_LNK_DATA_DIR}:/var/lib/${TMP_DC_PSQ_SETUP_APP_MARK}/data ${TMP_DC_PSQ_SETUP_LNK_ETC_DIR}/app:/etc/${TMP_DC_PSQ_SETUP_APP_MARK}" "" $([[ -z "${TMP_DC_PSQ_SETUP_IMG_SNAP_TYPE}" ]] && echo true)
    # fi
...
function exec_step_dc_library_postgres() {
    ...
    local TMP_DC_PSQ_SETUP_LNK_ETC_DIR=${DOCKER_APP_ATT_DIR}/${TMP_DC_PSQ_SETUP_IMG_MARK_NAME}/${TMP_DC_PSQ_SETUP_CTN_VER}

    ## 统一标记名称(存在于安装目录的真实名称)
    local TMP_DC_PSQ_SETUP_WORK_MARK="work"
    local TMP_DC_PSQ_SETUP_LOGS_MARK="logs"
    local TMP_DC_PSQ_SETUP_DATA_MARK="data"
    local TMP_DC_PSQ_SETUP_ETC_MARK="etc"
    local TMP_DC_PSQ_SETUP_APP_MARK="postgresql"

    ## 安装后的真实路径（此处依据实际路径名称修改）
    local TMP_DC_PSQ_SETUP_WORK_DIR=${TMP_DC_PSQ_SETUP_DIR}/${TMP_DC_PSQ_SETUP_WORK_MARK}
    local TMP_DC_PSQ_SETUP_LOGS_DIR=${TMP_DC_PSQ_SETUP_DIR}/${TMP_DC_PSQ_SETUP_LOGS_MARK}
    local TMP_DC_PSQ_SETUP_DATA_DIR=${TMP_DC_PSQ_SETUP_DIR}/${TMP_DC_PSQ_SETUP_DATA_MARK}
    local TMP_DC_PSQ_SETUP_ETC_DIR=${TMP_DC_PSQ_SETUP_DIR}/${TMP_DC_PSQ_SETUP_ETC_MARK}
...
function boot_build_dc_library_postgres() {
    ...
    local TMP_DC_PSQ_SETUP_PRE_ARG_ENVS="--env=TZ=Asia/Shanghai --env=POSTGRES_PASSWORD=${TMP_DC_PSQ_SETUP_DB_PASSWD}"
