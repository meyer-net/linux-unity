...
function boot_build_dc_library_mariadb() {
    ...
    ## 标准启动参数
    local TMP_DC_MDB_SETUP_PRE_ARG_MOUNTS="--volume=/etc/localtime:/etc/localtime:ro --volume=$(which yq):/usr/bin/yq --volume=$(which gum):/usr/bin/gum --volume=$(which pup):/usr/bin/pup"
