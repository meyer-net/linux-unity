...
function formal_dc_goharbor_harbor() {
    ...
    path_not_exists_link "${DOCKER_SETUP_DIR}/etc/${TMP_DC_CPS_HB_SETUP_IMG_MARK_NAME}/${TMP_DC_HB_SETUP_CTN_VER}" "" "${TMP_DC_HB_SETUP_LNK_ETC_DIR}"
    #### /mountdisk/data/docker/containers/${CTN_ID} -> /mountdisk/etc/docker_apps/goharbor_harbor/imgver111111/container
...
function exec_step_dc_goharbor_harbor() {
    # 变量覆盖特性，其它方法均可读取
	## 执行传入参数
	local TMP_DC_HB_SERVICE_NODE=${1}
    ...
    ## 统一编排到的路径
    local TMP_DC_HB_SETUP_LNK_ETC_DIR=${DOCKER_APP_ATT_DIR}/${TMP_DC_CPL_HB_SETUP_MARK_NAME}/${TMP_DC_CPL_HB_MARK_VER}/${TMP_DC_HB_CURRENT_IMG_MARK_NAME}
    ...
    local TMP_DC_HB_SETUP_WORK_DIR=${TMP_DC_CPL_HB_SETUP_DIR}/${TMP_DC_HB_CURRENT_IMG_MARK_NAME}/${TMP_DC_HB_SETUP_WORK_MARK}
    local TMP_DC_HB_SETUP_LOGS_DIR=${TMP_DC_CPL_HB_SETUP_DIR}/${TMP_DC_HB_CURRENT_IMG_MARK_NAME}/${TMP_DC_HB_SETUP_LOGS_MARK}
    local TMP_DC_HB_SETUP_DATA_DIR=${TMP_DC_CPL_HB_SETUP_DIR}/${TMP_DC_HB_CURRENT_IMG_MARK_NAME}/${TMP_DC_HB_SETUP_DATA_MARK}
    local TMP_DC_HB_SETUP_ETC_DIR=${TMP_DC_CPL_HB_SETUP_DIR}/${TMP_DC_HB_CURRENT_IMG_MARK_NAME}/${TMP_DC_HB_SETUP_ETC_MARK}
    
    echo_style_wrap_text "Starting 'execute step' <${TMP_DC_CPL_HB_SETUP_NAME}>:[${TMP_DC_CPL_HB_MARK_VER}]('${TMP_DC_HB_CURRENT_IMG_MARK_NAME}'), hold on please"
...
# x4-执行composer信息至exec_step_dc_goharbor_harbor（形成启动后才可抽取目录信息）
...
echo "${@}"
    function _setup_dc_goharbor_harbor_cp_compose() {
        echo "${TMP_SPLITER2}"
        echo_style_text "View the 'compose copy'↓:"
    ...
    # 创建安装目录(纯属为了规范)
    soft_path_restore_confirm_pcreate ${TMP_DC_HB_SETUP_COMPOSE_DIR} "_setup_dc_goharbor_harbor_cp_compose"
...
# 参数3：软件解压路径
# 参数4：软件版本
function conf_check_compile_dc_goharbor_harbor() {
	# 变量覆盖特性，其它方法均可读取
	## 执行传入参数
    ...
    # 重装/更新/安装
    soft_docker_compose_check_upgrade_action "goharbor/prepare" "${TMP_DC_CPL_HB_SETUP_VER}" "bash prepare" "bash install.sh --with-chartmuseum" "exec_compose_step_dc_goharbor_harbor"
    return $?
}
...
function download_package_dc_goharbor_harbor() {
    ...
    # 选择及下载安装版本
    soft_setup_docker_git_wget "${1}" "${1}" "https://github.com/${1}/releases/download/v%s/harbor-offline-installer-v%s.tgz" "1.10.17" "conf_check_compile_dc_goharbor_harbor"
